...
public class Q5 {
	public static void main(String[] args) {
		...
		for (int month = 1; month <= 12; month++) {
			...
			System.out.printf("%22s\n", monthInWords + ", " + year); //We print each month and the year given.
			System.out.println("____________________________");
			System.out.printf("%4s%4s%4s%4s%4s%4s%4s\n", "Sun","Mon","Tue", "Wed", "Thu", "Fri", "Sat");
			//the above statement is used to space the days of the week.
			...
			for(int j = 1; j <= numberOfDaysOfTheMonth; j++) {// we print each day of the month as gotten from the function.
				...
				if(count % 7 == 0) {
					System.out.println(); //once the line printed is up to 7 characters, we move to the next line.
				}
			}	
			System.out.println(); //we print a new line
			System.out.println();//we print a new line to help space each month output.
